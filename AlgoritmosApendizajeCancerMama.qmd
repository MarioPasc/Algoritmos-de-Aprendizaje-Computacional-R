...
double_cross_validation_rpart <- function(data, target_name, outer, inner, cp_values, minsplit_values, variables, threshold, seed) {
  ...
  for (outer_index in seq_along(outer_folds)) {
    ...
    best_model <- rpart(formula = as.formula(paste(target_name, "~", paste(variables, collapse = "+"))),
                       data = outer_test_data, control = rpart.control(cp = best_cp, minsplit = best_minsplit))
...
process_experiment_data <- function(raw_data) {
  ...
  performance_metrics <- performance_metrics %>%
    mutate(AUC = as.numeric(AUC),
           ...
           FP = as.numeric(FP),
           FN = as.numeric(FN),
           Best_Size = as.numeric(Best_Size),
           Best_Decay = as.numeric(Best_Decay),
           Seed = as.factor(seed))
...
generate_inner_outer_auc_comparison <- function(inner_fold_metrics, outer_fold_metrics) {
  # Calcular AUC medio para inner folds
  mean_inner_auc <- inner_fold_metrics %>%
    group_by(Fold, Seed) %>%
    summarize(mean_AUC = mean(AUC, na.rm = TRUE))
  
  # Calcular AUC medio para outer folds
  mean_outer_auc <- outer_fold_metrics %>%
    group_by(Fold, Seed) %>%
    summarize(mean_AUC = mean(AUC, na.rm = TRUE))
  
  # Unir los dataframes por Fold y Seed
  comparison_data <- merge(mean_inner_auc, mean_outer_auc, by = c("Fold", "Seed"), suffixes = c("_inner", "_outer"))
...
generate_outer_auc_violin_plot <- function(outer_fold_metrics) {
  # Calcular AUC medio por outer fold y semilla
  mean_outer_auc_by_seed <- outer_fold_metrics %>%
    group_by(Fold, Seed) %>%
    summarize(mean_AUC = mean(AUC, na.rm = TRUE))
  
  # Colores para cada fold
  colors <- RColorBrewer::brewer.pal(n = length(unique(mean_outer_auc_by_seed$Fold)), name = "Set1")
  
  # Crear la gráfica de violín
  fig <- plot_ly(data = mean_outer_auc_by_seed, y = ~mean_AUC, x = ~factor(Fold), type = 'violin', 
                 box = list(visible = TRUE), meanline = list(visible = TRUE), points = 'all', spanmode = 'hard',
                 color = ~factor(Fold), colors = colors) %>%
    layout(
      title = 'Distribución del AUC por Outer Fold y Semilla',
      xaxis = list(title = 'Fold'),
      yaxis = list(title = 'AUC Medio')
    )
...
```{r}
ANN_StepAUC_Metrics <- process_experiment_data(performance_ANN_stepAUC)
...
```{r}
generate_hyperparameter_stability_analysis <- function(performance_metrics, hyperparameters) {
  ...
  if (length(hyperparameters) == 2) {
    ...
    # Crear el gráfico de violín para mostrar la varianza de cada hiperparámetro
    violin_plot_x <- plot_ly(data = performance_metrics, y = ~get(hyper_x), type = 'violin', box = list(visible = TRUE), meanline = list(visible = TRUE)) %>%
      layout(
    ...
    violin_plot_y <- plot_ly(data = performance_metrics, y = ~get(hyper_y), type = 'violin', box = list(visible = TRUE), meanline = list(visible = TRUE)) %>%
      layout(
        title = paste('Distribución del Hiperparámetro:', hyper_y),
        yaxis = list(title = hyper_y),
        xaxis = list(title = hyper_y)
      )
    ...
    # Crear el gráfico de violín para mostrar la varianza del único hiperparámetro
    violin_plot_x <- plot_ly(data = performance_metrics, y = ~get(hyper_x), type = 'violin', box = list(visible = TRUE), meanline = list(visible = TRUE)) %>%
      layout(
        title = paste('Distribución del Hiperparámetro:', hyper_x),
        yaxis = list(title = hyper_x),
...
result <- generate_hyperparameter_stability_analysis(ANN_StepAUC_Metrics$performance_metrics, hyperparameters)
...
```{r}
generate_variable_stability_by_seed <- function(performance_metrics) {
  # Expandir las variables en filas individuales
  variable_data <- performance_metrics %>%
    separate_rows(Best_Variables, sep = ",") %>%
    group_by(Seed, Best_Variables) %>%
    summarize(count = n(), .groups = 'drop')
  
  # Obtener la lista completa de variables
  all_variables <- unique(variable_data$Best_Variables)
  
  # Generar una gráfica de barras por cada semilla
  unique_seeds <- unique(variable_data$Seed)
  plots <- list()
  
  # Definir la paleta de colores
  colors <- brewer.pal(n = length(unique_seeds), name = "Set1")
  
  # Encontrar el máximo valor para el eje Y
  max_y_value <- max(variable_data$count, na.rm = TRUE)
  
  for (i in seq_along(unique_seeds)) {
    seed <- unique_seeds[i]
    seed_data <- filter(variable_data, Seed == seed)
    
    # Asegurarse de que todas las variables estén presentes, incluso si no fueron seleccionadas
    seed_data <- seed_data %>%
      complete(Best_Variables = all_variables, fill = list(count = 0)) %>%
      arrange(Best_Variables)
    
    fig <- plot_ly(data = seed_data, x = ~Best_Variables, y = ~count, type = 'bar', 
                   marker = list(color = colors[i])) %>%
      layout(
        title = paste('Frecuencia de Selección de Variables para Semilla', gsub("seed_", "", seed)),
        xaxis = list(title = 'Variable'),
        yaxis = list(title = 'Frecuencia', range = c(0, max_y_value))
      )
    plots[[as.character(seed)]] <- fig
  }
...
```{r}
generate_variable_stability_overall <- function(performance_metrics) {
  # Expandir las variables en filas individuales
  variable_data <- performance_metrics %>%
    separate_rows(Best_Variables, sep = ",") %>%
    group_by(Best_Variables) %>%
    summarize(count = n(), .groups = 'drop') %>%
    mutate(percentage = count / sum(count) * 100)
  
  # Crear el gráfico de barras
  fig <- plot_ly(data = variable_data, x = ~Best_Variables, y = ~percentage, type = 'bar', 
                 marker = list(color = 'blue')) %>%
    layout(
      title = 'Frecuencia Total de Selección de Variables',
      xaxis = list(title = 'Variable'),
      yaxis = list(title = 'Frecuencia (%)')
    )
  
  return(list(plot = fig, dataframe = variable_data))
}
...
```{r}
# Uso de la función con los datos de ejemplo
# Asumiendo que performance_metrics_combined está generado y disponible
plots_by_seed <- generate_variable_stability_by_seed(ANN_StepAUC_Metrics$performance_metrics)
for (seed in names(plots_by_seed)) {
  print(paste("Gráfica para semilla:", seed))
  print(plots_by_seed[[seed]])
}

# Uso de la función con los datos de ejemplo
# Asumiendo que performance_metrics_combined está generado y disponible
result_overall <- generate_variable_stability_overall(ANN_StepAUC_Metrics$performance_metrics)
fig_overall <- result_overall$plot
variable_stability_df <- result_overall$dataframe

# Mostrar la visualización y el dataframe
fig_overall
variable_stability_df
```
