...
performance_adaboost_lasso <- evaluate_with_seeds_AdaBoost_Lasso(
...
resultadosAparentesDT <- evaluate_aparent_performance_model_rpart(data = data_factor, target_var = "PCR",
                                                             ...
                                                             vars = ".",
                                                             threshold = .35)
save(resultadosAparentesDT, file = "RData_Files_Algorithms/DecisionTree_Aparente.RData")
...
save(performance_rpart_todasvariables, file = "RData_Files_Algorithms/DecisionTree_TodasVariables.RData")
save(performance_rpart_Asociacion, file = "RData_Files_Algorithms/DecisionTree_Asociacion.RData")
...
save(performance_rpart_StepAUC, file = "RData_Files_Algorithms/DecisionTree_StepAUC.RData")
```
...
save(performance_rpart_lasso, file = "RData_Files_Algorithms/DecisionTree_Lasso.RData")
```
...
## 2CV con todas las Variables + 2CV Filtrado
...
performance_RandomForest_Asociacion <- evaluate_with_seeds_RandomForest_TodasVariables_Asociacion(
  evaluation_function = double_cross_validation_randomforest,
  data = data_factor,
  outer = 5,
  inner = 2,
...
save(performance_RandomForest_todasvariables, file = "RData_Files_Algorithms/RandomForest_TodasVariables.RData")
save(performance_RandomForest_Asociacion, file = "RData_Files_Algorithms/RandomForest_Asociacion.RData")
...
save(performance_RandomForest_StepAUC, file = "RData_Files_Algorithms/RandomForest_StepAUC.RData")
```
...
```{r}
double_cross_validation_randomforest_Lasso <- function(data, target_name, outer, inner, ntree_values, mtry_values, variables, threshold, seed) {
  set.seed(seed)
  performance_metrics <- data.frame(Fold = integer(), TP = integer(), TN = integer(),
                                    FP = integer(), FN = integer(), AUC = numeric(),
                                    Best_Ntree = integer(), Best_Mtry = integer(), 
  ...
  # Convertir las variables del dataset con model.matrix
  data[[target_name]] <- factor(data[[target_name]], levels = c(0, 1))  # Convertir a factor
  target <- as.numeric(data[[target_name]]) - 1
  features <- data[, setdiff(names(data), target_name)]
  data_matrix <- as.data.frame(model.matrix(~ ., data=features)[,-1])  # Eliminar el intercepto
  data_matrix[[target_name]] <- target
  ...
  for (outer_index in seq_along(outer_folds)) {
      ...
      while (improved && length(vars) > 1) {
        ...
        for (params in 1:nrow(grid)) {
          ...
          for (inner_index in seq_along(inner_folds)) {
            ...
            result <- tryCatch({
              model <- randomForest(as.formula(formula), data = inner_train_data, ntree = grid$ntree[params], mtry = grid$mtry[params])
    ...
    FP <- ifelse(nrow(confusion) >= 1 && ncol(confusion) >= 2, confusion[1, 2], 0)
    FN <- ifelse(nrow(confusion) >= 2 && ncol(confusion) >= 1, confusion[2, 1], 0)
    roc_curve <- roc(outer_test_data[[target_name]], predictions)
    auc <- roc_curve$auc
    
    outer_metrics <- setNames(c(outer_index, TP, TN, FP, FN, auc, best_ntree, best_mtry, paste(best_vars, collapse = ",")),
                              names(performance_metrics))
    performance_metrics <- rbind(performance_metrics, as.data.frame(t(outer_metrics)))
  }
  
  list(outer_performance = performance_metrics, inner_performance = inner_fold_metrics)
}
...
performance_RandomForest_Lasso <- evaluate_with_seeds_RandomForest_Lasso(
  evaluation_function = double_cross_validation_randomforest_Lasso,
  data = data_factor,
  outer = 5,
  inner = 2,
  target_var = "PCR",
...
#save(performance_RandomForest_Lasso, file = "RData_Files_Algorithms/RandomForest_Lasso.RData")
```
...
double_cross_validation_knn <- function(data, target_name, outer, inner, ks, ls, variables, threshold, seed) {
  ...
  for (outer_index in seq_along(outer_folds)) {
    ...
    for (inner_index in seq_along(inner_folds)) {
      ...
      predictions <- knn(train = best_model$train_x, test = inner_test_data[, variables, drop = FALSE], cl = best_model$train_y, k = best_k, prob = TRUE)
      probabilities <- attr(predictions, "prob")
      probabilities <- ifelse(predictions == "1", probabilities, 1 - probabilities)
      roc_curve <- roc(as.numeric(as.character(inner_test_data[[target_name]])), probabilities)
      inner_auc_vals[inner_index] <- roc_curve$auc
...
double_cross_validation_svm <- function(data, target_name, outer, inner, kernels, costs, variables, threshold, seed) {
  ...
  for (outer_index in seq_along(outer_folds)) {
    ...
    for (inner_index in seq_along(inner_folds)) {
      ...
      roc_curve <- roc(inner_test_data[[target_name]], prob)
      inner_auc_vals[inner_index] <- roc_curve$auc
